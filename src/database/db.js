...
// Crear la base de datos (en este caso, 'my-database.db')
   ...
   /*     db.exec("ALTER TABLE loans ADD COLUMN payment_gen_date DATE;")
     ...
     db.exec("CREATE TABLE IF NOT EXISTS  LoansInfo(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, payment_interval INTEGER ,gains REAL,  label TEXT DEFAULT 'prestamo',    amount REAL DEFAULT 0.00,    interes_percentage INTEGER DEFAULT  0,    installments INTEGER DEFAULT 1,lender_id INTEGER,aproved_date DATE DEFAULT CURRENT_DATE, create_at DATE DEFAULT CURRENT_TIMESTAMP,   state TEXT DEFAULT 'active',    client_id INTEGER NOT NULL,    payment_method TEXT DEFAULT 'cash',    FOREIGN KEY (client_id) REFERENCES clients(id) ON DELETE CASCADE,    FOREIGN KEY (lender_id) REFERENCES users(id) ON DELETE CASCADE)")
     ...
     db.exec("CREATE TABLE  clients(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,name TEXT ,lastname TEXT,email TEXT, nickname TEXT NOT NULL, phonenumber TEXT,address TEXT, create_at DATE DEFAULT CURRENT_TIMESTAMP, rate INTEGER DEFAULT 100, notes TEXT);")
   db.exec("DROP TABLE IF EXISTS users;DROP TABLE IF EXISTS clients;DROP TABLE IF EXISTS loans;DROP TABLE IF EXISTS payments;DROP TABLE IF EXISTS bank_account;")
    ...
    db.exec("CREATE TABLE  clients(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,name TEXT ,lastname TEXT, nickname TEXT NOT NULL, phonenumber TEXT,address TEXT, create_at DATE DEFAULT CURRENT_TIMESTAMP, rate INTEGER DEFAULT 100, notes TEXT);")
 ...
 */
   // db.exec("CREATE TABLE IF NOT EXISTS  bank_account(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,    cbu TEXT,   alias TEXT,    user_id INTEGER,  client_id INTEGER,    FOREIGN KEY (client_id) REFERENCES clients(id) ON DELETE CASCADE,    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE")
